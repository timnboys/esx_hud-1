...
function setIdentity(identifier, data, blood, callback)
	MySQL.Async.execute('UPDATE `users` SET `firstname` = @firstname, `lastname` = @lastname, `dateofbirth` = @dateofbirth, `sex` = @sex, `height` = @height, `blood` = @blood WHERE identifier = @identifier', {
		['@identifier']		= identifier,
...
AddEventHandler('esx_identity:setIdentity', function(data, myIdentifiers, blood)
	setIdentity(myIdentifiers.steamid, data, blood, function(callback)
		if callback then
