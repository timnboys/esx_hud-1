...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Musisz pierw stworzyć postać aby móc grać' } })
	end
end)
...
RegisterNUICallback('register', function(data, cb)
	...
	if reason == "" then
		blood = exports["esx_bloodtype"]:randomBlood()
		TriggerServerEvent('esx_identity:setIdentity', data, myIdentifiers, blood)
		EnableGui(false)
		Citizen.Wait(500)
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Twoja nazwa jest za krótka bądź za długa'
	end
	...
	if count ~= nameLength then
		return 'Twoja nazwa zawiera znaki specjalne niedozwolone na serwerze'
	end
	...
	if spacesInName > 2 then
		return 'Twoje imie zawiera conajmniej dwie spacje'
	end

	if spacesWithUpper ~= spacesInName then
		return 'Twoje imie musi zaczynać się z wielkiej litery'
	end
